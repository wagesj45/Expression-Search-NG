...
var ExpressionSearchTokens = {
  tokenDict: {
    from: ['f'], fromre: ['fr'], to: ['t', 'toorcc'], tore: ['tr'], tonocc: ['tn'], cc: ['c'], bcc: ['bc'], only: ['o'], subject: ['s'], size: ['si', 'larger'], smaller: ['sm'],
    ...
    fromto: ['ft', 'ftc', 'fromtocc', 'alladdresses'], xnote: ['xn']
  },
  ...
  slowTokens: ['body', 'filename', 'bodyre', 'xnote'], // tokens that should search later than others
...
function ADVANCE_TOKEN() {
  ...
  if (this.str[0] == '(' || this.str[0] == ')' ||
    ...
    this.next_token = {
      kind: (tok == '-') || (tok == '^') ? "op" : "g",  //allow not and begins with
      tok: tok
    };
    return;
  }
  ...
  if (e.kind == 'op' && e.tok == '-') {
    ...
    if (under.kind == 'op') {
      if (under.tok == 'and' || under.tok == 'or')
        return expr_demorgan({
          kind: 'op', tok: under.tok == 'and' ? 'or' : 'and',
